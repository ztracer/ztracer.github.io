...
>[学长的期末复习（终点略有不同）](https://blog.csdn.net/weixin_47102975/article/details/126817255)
...
- 保护模式下的内存管理：分段（必须）+ 分页（可选）
  - 物理地址是CPU访问的实际内存位置
  - CPU的内存管理单元(MMU)透明地将虚拟地址(逻辑地址)转换为物理地址![](/img/inPost/SoftwareSecurity/保护模式下的内存管理.png)
